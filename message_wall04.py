...
def message_wall_app(environ, start_response):
    ...
    if environ['REQUEST_METHOD'] == 'POST':
        ...
        cursor.execute("insert into messages (user, message, ts) values "
                       "(:user, :messages, :timestamp)", form_vals)
    path_vals = environ['PATH_INFO'][1:].split("/")
    user,*tag = path_vals
    cursor.execute("""select * from messages where user like ? or message like ? order by ts""",
                   (user, "@" + user + "%"))
...
conn = sqlite3.connect("/home/apickeral/messagefile")
cursor = conn.cursor()
httpd.serve_forever()
